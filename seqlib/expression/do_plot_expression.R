...
#+geom_line(aes(x=group,y=fpkm,color=GeneName))+geom_point(aes(x=group,y=fpkm,group=group,color=GeneName),size=1.5)
	...
	g=g+stat_summary(aes(x=group,y=fpkm,fill=GeneName),fun.y=mean,geom='bar',position='dodge')+stat_summary(geom="errorbar",width=.5,fun.data="mean_cl_boot",position=position_dodge(width=.93),aes(x=group,y=fpkm,color=GeneName,group=GeneName))+theme_bw()+scale_fill_brewer(palette="Set1")+scale_color_brewer(palette="Set1")+theme(legend.title=element_blank(),legend.key=element_blank(),legend.position="bottom",legend.direction='horizontal',axis.text.x=element_text(angle=45,hjust=1,vjust=1))+guides(color=guide_legend(nrow=2),fill=guide_legend(nrow=2))+scale_x_continuous("",breaks=xbreaks,labels=xlabels)+scale_y_continuous("FPKM")
